 ...
 * @Last Modified time: 2021-09-20 01:57:27
 */
...
export const SelectOptGroupFC = ({
  ...
  return (
    ...
    >
      {data.map((dataItem) => {
        if (dataItem) {
          const { [labelName]: labelValue, [optionsName]: optionsValue } = dataItem;
          return (
            <OptGroup key={labelValue} label={labelValue}>
              {getArray(optionsValue).map((item) => {
                if (item) {
                  const {
                    [optionValue]: value,
                    [optionText]: text,
